...
class BlumBot:
    def __init__(self, thread, account, session, proxy):
        ...
        self.refresh_token = ''
        self.referralToken = "ref_Ot9UB2x0iM"
    ...
    async def login(self):
        ...
        try:
            ...
            resp_json = await resp.json()
            if "token" not in str(resp_json):
                username=resp_json.get("username")
                r = await self.session.post("https://gateway.blum.codes/v1/user/username/check",json=resp_json, proxy=self.proxy, ssl=False)
                json2_data= {"query": await self.get_tg_web_data(),
                            "username":username,
                            "referralToken":self.referralToken.split("_")[1]}
                resp = await self.session.post("https://gateway.blum.codes/v1/auth/provider/PROVIDER_TELEGRAM_MINI_APP",
                                           json=json2_data, proxy=self.proxy, ssl=False)
                resp_json = await resp.json()
    ...
    async def get_tg_web_data(self):
        ...
        async for message in self.client.get_chat_history('BlumCryptoBot'):
        ...
        if not start_command_found:
            await self.client.send_message("BlumCryptoBot", f"/start {self.referralToken}")
